...
def main():
    ...
    joblib.dump(X, "saved_data/\
                    saved_tf_idf_weighted_document_term_matrix_from_tfidfvectorizer.pkl")
    ...
    #test_classifier.test_data_from_json_file()
    
    '''
    for category in predicted:
        print('%d  %s' % (corpus_test_index, category))
        corpus_test_index += 1
    '''
    '''
    ...
    '''
    X_new_counts = vectorizer.transform(corpus_test)
    print "X_new_counts = ", X_new_counts
    X_new_tfidf = vectorizer.transform(X_new_counts)
    print "X_test = ", X_test
    print "X_new_counts = ", X_new_counts
    print "X_new_tfidf", X_new_tfidf
    
    predicted = svm.predict(X_new_tfidf)
    for doc, category in zip(corpus_test, predicted):
        print('%r => %s' % (doc, y[category]))
    '''
    ''' print json 
    print data[0]["id"]
    print data[0]["body"]
